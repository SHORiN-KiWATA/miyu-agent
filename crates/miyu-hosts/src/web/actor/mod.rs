                ...
                tokio::task::spawn_local(miyu_base::sandbox::with_live_sandbox(
                    ...
                    miyu_base::workspace::with_workspace(
                        ...
                        miyu_base::workspace::with_session(
                            session_id,
                            miyu_base::workspace::with_origin_tty(
                                origin_tty.map(|origin| *origin),
                                miyu_base::workspace::with_platform_sender(
                                    platform_sender,
                                    miyu_base::workspace::with_turn_origin(*turn_origin, task),
                                ),
                            ),
                        ),
                    ),
                ...
                let forward = move |event: AgentEvent| -> Result<()> {
                    ...
                    let frame = match event {
                        ...
                        AgentEvent::CompactChunk(chunk) => (chunk.kind
                            == miyu_core::llm::ChatStreamKind::Content)
