...
fn worker_wanted(voice: &miyu_base::config::VoiceConfig) -> bool {
...
pub(crate) fn on_config_reload(
    state: &DaemonState,
    previous: &miyu_base::config::VoiceConfig,
    next: &miyu_base::config::VoiceConfig,
) {
...
pub(crate) async fn handle_voice_attach(
    ...
    *ATTACH.lock().unwrap() = Some(tx);
    miyu_core::ipc::send(stream, &miyu_core::ipc::Frame::Ack).await?;
    loop {
        tokio::select! {
            signal = rx.recv() => {
...
fn notify(state: &DaemonState, title: &str, body: &str) {
    ...
    if !cfg!(target_os = "linux") {
        miyu_base::notify::notify(title, body);
        return;
    }
            ...
            std::thread::Builder::new()
                ...
                .spawn(move || {
                    ...
                    for (title, body) in rx {
                        last = miyu_base::notify::notify_replacing(&title, &body, last);
                    }
                })
                .ok();
            tx
...
fn resolve_voice_session(state: &DaemonState) -> Result<String> {
    ...
    let record = state.state_store.create_session(
        &persona,
        miyu_base::i18n::text("Voice chat", "语音会话"),
...
fn cancel_active_run(state: &DaemonState) {
    ...
    tokio::spawn(async move {
        let outcome = async {
            let mut stream = miyu_core::ipc::connect(&socket).await?;
            miyu_core::ipc::send(
                &mut stream,
                &miyu_core::ipc::Request::new(miyu_core::ipc::Command::Cancel { run_id }),
            )
            .await?;
            let _ = miyu_core::ipc::receive::<miyu_core::ipc::Frame>(&mut stream).await;
            anyhow::Ok(())
        }
        .await;
...
async fn run_voice_turn(state: &DaemonState, content: String) -> Result<()> {
    ...
    let outcome = async {
        ...
        miyu_core::ipc::send(
            &mut stream,
            &miyu_core::ipc::Request::new(miyu_core::ipc::Command::StartTurn {
        ...
        loop {
            let Some(frame) = miyu_core::ipc::receive::<miyu_core::ipc::Frame>(&mut stream).await?
            else {
                return Ok((reply, "disconnected"));
            };
            match frame {
                miyu_core::ipc::Frame::Accepted { run_id, .. } => {
                    *ACTIVE_RUN.lock().unwrap() = Some(run_id.clone());
                    my_run = Some(run_id);
                }
                miyu_core::ipc::Frame::Event { kind, data, .. } => match kind.as_str() {
                    "assistant.delta" => {
                ...
                miyu_core::ipc::Frame::Error { message, .. } => anyhow::bail!("{message}"),
                _ => {}
            }
            let _ = &my_run;
        }
...
pub(crate) async fn speak_with(
    state: &DaemonState,
    text: &str,
    override_tts: Option<miyu_base::config::VoiceTtsConfig>,
) -> Result<()> {
    let tts =
...
async fn synthesize_to_cache(
    state: &DaemonState,
    tts: &miyu_base::config::VoiceTtsConfig,
    text: &str,
) -> Result<PathBuf> {
...
pub(crate) async fn handle_voice_speak(
    ...
    if let Err(message) = wait_attached(state, 20_000).await {
        miyu_core::ipc::send(stream, &miyu_core::ipc::Frame::error(message)).await?;
        return Ok(());
    }
    match speak_with(state, &text, override_tts).await {
        Ok(()) => miyu_core::ipc::send(stream, &miyu_core::ipc::Frame::Ack).await?,
        Err(error) => {
            miyu_core::ipc::send(stream, &miyu_core::ipc::Frame::error(format!("{error:#}")))
                .await?
        }
    }
    Ok(())
        ...
        tokio::select! {
            event = rx.recv() => match event {
                Some(DictationRelay::Utterance(text)) => {
                    miyu_core::ipc::send(stream, &miyu_core::ipc::Frame::Event {
                        id: 0,
                ...
                Some(DictationRelay::Ended) | None => {
                    let _ = miyu_core::ipc::send(stream, &miyu_core::ipc::Frame::Event {
                        id: 0,
...
/// [`crate::runtime::VoicePort`] 的 daemon 实现:全部转发到本模块的函数。工具层
/// (`speak` / `end_voice_chat` / `send_qq_message`)与平台层(`send_voice_message`、
/// QQ 入站语音转写)经端口调用,不再 `use crate::web`。随 `install_state` 一起装入。
pub(crate) struct VoiceBridgePort;

impl crate::runtime::VoicePort for VoiceBridgePort {
    fn tts_available(&self) -> bool {
        tts_available()
    }

    fn stt_available(&self) -> bool {
        DAEMON_STATE.get().is_some_and(stt_available)
    }
    ...
    fn speak(&self, text: String) -> futures_util::future::BoxFuture<'static, Result<()>> {
        Box::pin(async move { speak_from_tool(&text).await })
    }

    fn synthesize_file(
        &self,
        text: String,
    ) -> futures_util::future::BoxFuture<'static, Result<PathBuf>> {
        Box::pin(async move { synthesize_for_platform(&text).await })
    }
    ...
    fn transcribe_wav(
        ...
        Box::pin(async move {
            let state = DAEMON_STATE.get().context("语音转写只能在 daemon 里用")?;
            transcribe_wav(state, &wav).await
        })
    }
}
