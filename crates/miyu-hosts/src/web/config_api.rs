...
pub(in crate::web) struct UpdateConfigRequest {
    ...
    #[serde(default)]
    #[allow(dead_code)] // HTTP 请求体的 DTO 字段
...
pub(in crate::web) async fn update_config(
    ...
    let renames =
        miyu_base::config::detect_provider_renames(&current.providers, &candidate.providers);
...
pub(in crate::web) fn rebuild_for_config(
    ...
    let migrated_previous_scope = persona_changes
        ...
        .find_map(|(old_name, new_name)| {
            (miyu_base::config::persona_scope_name(old_name) == previous_scope)
                .then(|| {
                    new_name
                        .as_deref()
                        .map(miyu_base::config::persona_scope_name)
                })
                .flatten()
        })
    ...
    *config = next_config.clone();
    ...
    for (old_name, new_name) in &persona_changes {
        if new_name.is_none() {
            if let Err(error) =
                state_store.delete_persona_scope(&miyu_base::config::persona_scope_name(old_name))
            {
                tracing::warn!(
                    %error,
                    %old_name,
