...
fn tool_call_succeeded(output: &str) -> bool {
    !output.trim_start().starts_with("tool error:")
        && miyu_engine::agent::tool_output_succeeded(output)
}
...
impl From<miyu_core::state::ToolFlowRound> for SafeToolRound {
    fn from(round: miyu_core::state::ToolFlowRound) -> Self {
        Self {
            remote: round.remote,
            ...
            calls: round
                ...
                .map(|call| SafeToolCall {
                    display_name: miyu_engine::tools::readable_tool_name(&call.name),
...
pub(in crate::web) struct SafeTurn {
    ...
    pub(in crate::web) question_exchanges: Vec<miyu_base::question::QuestionExchange>,
    /// 这一轮调过哪些工具、拿到什么结果。
    ///
...
impl From<miyu_core::state::RedoCandidate> for SafeRedoCandidate {
    fn from(candidate: miyu_core::state::RedoCandidate) -> Self {
        Self {
            turn_id: candidate.turn_id,
            revision: candidate.revision,
            input_id: candidate.input_id,
            input_kind: match candidate.input_kind {
                miyu_core::state::RedoInputKind::Initial => "initial",
                miyu_core::state::RedoInputKind::Followup => "followup",
            },
...
pub(in crate::web) fn redact_internal_assistant_text(value: &str) -> String {
    value
        .replace(miyu_core::state::pending_placeholder(), "")
        .replace(miyu_core::state::interrupted_text(), "")
}
