...
use miyu_base::config::{feature_catalog, AppConfig, PersonaManifest};
...
pub(crate) fn save_settings(
    ...
    if let Some(parent) = path.parent() {
        miyu_base::paths::ensure_private_dir(parent)?;
    }
...
pub(crate) fn create_or_update_persona(
    ...
    let dir = personas_root(paths, username).join(slug);
    miyu_base::paths::ensure_private_dir(&paths.user_home_dir(username))?;
    miyu_base::paths::ensure_private_dir(&personas_root(paths, username))?;
    miyu_base::paths::ensure_private_dir(&dir)?;
