...
async fn command_catalog_carries_what_the_menu_needs() {
    ...
    let expected = miyu_core::slash_commands::web_commands()
        .into_iter()
        .map(|spec| spec.name)
        .collect::<Vec<_>>();
    let actual = commands
...
async fn web_memory_reset_all_clears_the_mode_it_was_asked_for() {
    ...
    let session = state
        .state_store
        .create_session(miyu_core::state::DEV_PERSONA, "dev memory", "user", None)
        .unwrap();

    let normal = miyu_core::memory::MemoryStore::new(&config, &paths);
    ...
    let dev = miyu_core::memory::MemoryStore::new(&dev_config, &paths);
    normal
        .remember_fact("普通模式记住 XMODIFIERS 这件事", "test")
        .unwrap();
    dev.remember_fact("开发模式记住 XMODIFIERS 这件事", "test")
        .unwrap();
    let recalled = |store: &miyu_core::memory::MemoryStore| {
        store
...
async fn web_memory_reset_clears_only_the_session_it_was_asked_for() {
    ...
    let store = miyu_core::memory::MemoryStore::new(&config, &paths);
    store
        .clone()
