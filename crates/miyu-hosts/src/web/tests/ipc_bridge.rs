...
fn the_three_repl_lanes_never_share_a_session() {
    ...
    let dev = store
        .ensure_repl_session(miyu_core::state::DEV_PERSONA)
        .unwrap();
...
async fn tool_bridge_executes_with_session_scope_and_depth_guard() {
    ...
    let denied = handle_session_command(
        ...
        IpcCommand::ToolCall {
            ...
            arguments: "{}".to_string(),
            origin: None,
            depth: miyu_base::workspace::MAX_BRIDGE_DEPTH,
        },
    )
    .await
    .unwrap_err();
...
async fn reset_memory_bumps_generation_in_the_requested_scope_only() {
    ...
    let dev_store = miyu_core::memory::MemoryStore::new(&config.dev_scoped(), &state.paths);
    dev_store.init().unwrap();
    let normal_store = miyu_core::memory::MemoryStore::new(&config, &state.paths);
...
async fn session_scoped_memory_reset_honours_the_named_session() {
    ...
    let target = state
        ...
        .unwrap();
    let config = state.manager.lock().unwrap().config.clone();
    let store = miyu_core::memory::MemoryStore::new(&config, &state.paths);
    store
        .clone()
...
async fn dev_sessions_resolve_by_id_and_list_under_dev_mode() {
    ...
    let dev = state
        .state_store
        .create_session(miyu_core::state::DEV_PERSONA, "编译修复", "user", None)
        .unwrap();
    ...
    assert_eq!(resolved.persona, miyu_core::state::DEV_PERSONA);
...
#[tokio::test]
async fn host_query_round_trips_over_ipc_with_a_grant() {
    let temp = tempfile::tempdir().unwrap();
    let state = DaemonState::for_test(test_paths(temp.path()), 8300).unwrap();
    async fn ask(state: &DaemonState, token: &str, method: &str) -> serde_json::Value {
        let (mut client, server) = tokio::net::UnixStream::pair().unwrap();
        let server_state = state.clone();
        let task = tokio::spawn(async move { handle_ipc_connection(server_state, server).await });
        ipc::send(
            &mut client,
            &IpcRequest::new(IpcCommand::HostQuery {
        ...
        task.await.unwrap().unwrap();
        match response {
            IpcFrame::AdminResult { data, .. } => data,
            other => panic!("unexpected frame: {other:?}"),
        }
    }
    let denied = ask(&state, "nope", "providers.list").await;
    assert_eq!(denied["ok"], false, "{denied}");
    assert_eq!(denied["error"]["code"], "permission_denied");

    crate::runtime::enable_host_grants();
    let grant = miyu_base::host_ports::issue_host_grant(&["providers.read".to_string()]).unwrap();
    let reply = ask(&state, grant.token(), "providers.list").await;
    assert_eq!(reply["ok"], true, "{reply}");
    assert!(reply["data"]["providers"]
        .as_array()
        .is_some_and(|providers| !providers.is_empty()));
    assert!(!reply.to_string().contains("api_key"));
    let info = ask(&state, grant.token(), "host.info").await;
    assert_eq!(
        info["error"]["code"], "permission_denied",
        "只签了 providers.read"
    );
    let token = grant.token().to_string();
    drop(grant);
    let expired = ask(&state, &token, "providers.list").await;
    assert_eq!(
        expired["error"]["code"], "permission_denied",
        "守卫掉落即作废"
    );
}
