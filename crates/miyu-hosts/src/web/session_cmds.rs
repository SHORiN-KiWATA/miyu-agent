...
pub(in crate::web) async fn handle_session_command(
    ...
    match command {
        IpcCommand::SetRequestLogging { enabled } => {
            ...
            miyu_core::llm::request_log::install_dir(state.paths.logs_dir());
            miyu_core::llm::request_log::set_enabled(enabled);
            Ok(json!({
                "enabled": enabled,
                "file": miyu_core::llm::request_log::current_file()
        ...
        IpcCommand::ResetMemory {
            ...
            let memory = miyu_core::memory::MemoryStore::new(&config, &state.paths);
            match scope {
        ...
        IpcCommand::ListSessions { mode } => {
            ...
            let current = if dev {
                store
                    .repl_session(miyu_core::state::DEV_PERSONA)
                    .ok()
                    .flatten()
                    .unwrap_or_default()
                    .into()
        ...
        IpcCommand::CreateSession {
            ...
            let kind = match kind.as_deref() {
                None | Some(miyu_core::state::USER_SESSION_KIND) => {
                    miyu_core::state::USER_SESSION_KIND
                }
                Some(miyu_core::state::ASK_SESSION_KIND) if !switch => {
            ...
            let record = store
                ...
                .map_err(|error| safe_error_message(&error))?;
            if kind == miyu_core::state::USER_SESSION_KIND {
        ...
        IpcCommand::ToolCall {
            ...
            let output = miyu_base::sandbox::with_sandbox(
                sandbox,
                miyu_base::workspace::with_workspace(
                    workspace,
                    miyu_base::workspace::with_session(
                        session_arc,
                        miyu_base::workspace::with_turn_origin(
                            turn_origin,
        ...
        IpcCommand::SetReplSession { target } => {
            ...
            if record.session_id != miyu_core::state::DEFAULT_SESSION_ID {
                store
...
pub(in crate::web) fn attach_owner_turn_tools(
    ...
    if mode == PersonaLane::Active && session_is_running_local_webui(state, session_id) {
        ...
        miyu_engine::tools::register_webui_share_tools(registry, config, state.state_store.clone());
    }
}
