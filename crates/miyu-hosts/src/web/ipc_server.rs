...
pub(in crate::web) fn start_ipc_server(
    state: &DaemonState,
) -> Result<(miyu_core::ipc::WebCoreLease, TokioJoinHandle<()>)> {
...
async fn dispatch_ipc_connection(
    ...
    match request.command {
            ...
            let attachments = images
                ...
                .map(|image| match image {
                    ImageAttachment::Binary { mime, data } => {
                        miyu_core::state::QueuedPromptAttachment::Binary {
                            mime,
                            data_base64: base64::engine::general_purpose::STANDARD.encode(data),
                        }
                    }
                    ImageAttachment::Path { path } => {
                        miyu_core::state::QueuedPromptAttachment::Path { path }
                    }
                })
                .collect();
        ...
        IpcCommand::HostQuery {
            ...
            let config = state.manager.lock().unwrap().config.clone();
            let outcome = match crate::runtime::host_grant_capabilities(&token) {
                None => Err(crate::runtime::HostQueryError {
                    code: "permission_denied",
                    message: "unknown or expired host token".to_string(),
                }),
                Some(granted) => crate::runtime::answer_host_query(
                    &granted,
                    &config,
                    &state.paths,
                    &method,
