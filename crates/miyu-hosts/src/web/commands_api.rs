//! WebUI 的斜杠命令平面。
//!
//! 命令清单来自 `miyu_core::slash_commands` 的同一张表（`GET /api/commands` 按
...
pub(in crate::web) async fn list_commands(
    ...
    let commands = miyu_core::slash_commands::web_commands()
        .into_iter()
        .map(|spec| WebCommand {
            name: spec.name,
...
pub(in crate::web) async fn session_goal_http(
    ...
    let goal = miyu_engine::tools::goal::session_goal_json(&store, &session_id);
    Ok(Json(goal))
}
...
pub(in crate::web) async fn reset_memory_http(
    ...
    let summary = miyu_core::memory::MemoryStore::new(&config, &state.paths)
...
pub(in crate::web) async fn reset_all_memory_http(
    ...
    require_local_web_session(&state, &headers, &session_id)?;
    let config = reset_memory_config(&state, &session_id, request.mode.as_deref());
    miyu_core::memory::MemoryStore::new(&config, &state.paths)
        .reset_all()
        .map_err(|error| ApiError::internal(safe_error_message(&error)))?;
