        ...
        .map(|image| {
            image.map(|image| match image {
                ImageAttachment::Binary { mime, data } => {
                    miyu_base::clipboard::PastedImage::Binary(
                        miyu_base::clipboard::ClipboardImage::new(mime, data),
                    )
                }
                ImageAttachment::Path { path } => miyu_base::clipboard::PastedImage::Path(path),
            })
        })
        .collect()
}
...
async fn run_turn_task_inner(
    ...
    let chat_outcome = match input {
        ...
        TurnTaskInput::Redo { candidate, prompts } => {
            ...
            let prompts = prompts
                .into_iter()
                .map(|prompt| miyu_engine::agent::RedoPromptInput {
