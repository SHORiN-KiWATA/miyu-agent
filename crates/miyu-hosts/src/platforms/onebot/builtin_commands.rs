...
pub(in crate::platforms::onebot) async fn execute_builtin_command(
    ...
    let response = match command {
        ...
        commands::ParsedPlatformCommand::ResetMemory => {
            ...
            match resolve_onebot_session(state, context, target, event) {
                ...
                Ok(session_id) => {
                    ...
                    match miyu_core::memory::MemoryStore::new(&context.config, &state.paths)
        ...
        commands::ParsedPlatformCommand::ResetAllMemory => {
            ...
            match miyu_core::memory::MemoryStore::new(&context.config, &state.paths).reset_all() {
...
pub(in crate::platforms::onebot) fn execute_models_command(
    ...
    let selected = match miyu_base::config::resolve_provider_model_argument(&choices, argument) {
    ...
    let mut route = next_config
        ...
        .cloned()
        .unwrap_or_else(|| miyu_base::config::PlatformModelRoute {
            conversation: miyu_base::config::PlatformConversationConfig {
                kind,
                id: conversation_id.clone(),
            },
            persona: miyu_base::config::PlatformPersonaOverride::default(),
            text_models_inheritance: miyu_base::config::PlatformModelPoolInheritance::default(),
            text_models: None,
            multimodal_models_inheritance: miyu_base::config::PlatformModelPoolInheritance::default(
            ),
