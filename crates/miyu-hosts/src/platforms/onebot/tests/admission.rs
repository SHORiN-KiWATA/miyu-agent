...
async fn same_conversation_judgements_reuse_parallel_turn_admission() {
    ...
    let state = test_web_state(temp.path(), 8300);
    {
        ...
        manager.config.platforms.qq.group_chats.allow_non_whitelist = true;
        manager.config.platforms.qq.session_parallel = true;
        manager.config.platforms.qq.session_limits = miyu_base::config::PlatformSessionLimits {
            running: 2,
            queued: 2,
        };
        manager
...
fn admission_matrix_uses_private_and_group_conversation_buckets() {
    ...
    let admin = admission_for_access(&config, None, Target::Group { group_id: 99 }, 100, 1);
    assert!(admin.allowed);
    assert!(admin.rate_key.is_none());
    assert!(admin.use_non_whitelist_text_models);

    let private_admin = admission_for_access(&config, None, Target::Private { user_id: 1 }, 100, 1);
    assert!(private_admin.allowed);
    assert!(!private_admin.use_non_whitelist_text_models);

    let private_whitelist =
        admission_for_access(&config, None, Target::Private { user_id: 2 }, 100, 2);
    ...
    let private_guest = admission_for_access(&config, None, Target::Private { user_id: 3 }, 100, 3);
    ...
    let group_whitelist =
        admission_for_access(&config, None, Target::Group { group_id: 10 }, 100, 2);
    ...
    let group_guest = admission_for_access(&config, None, Target::Group { group_id: 11 }, 100, 3);
    ...
    let privileged_group_guest =
        admission_for_access(&config, None, Target::Group { group_id: 11 }, 100, 2);
    ...
    assert!(!admission_for_access(&config, None, Target::Private { user_id: 3 }, 100, 3).allowed);
    assert!(!admission_for_access(&config, None, Target::Group { group_id: 11 }, 100, 3).allowed);
...
fn admission_materializes_the_effective_text_model_pool() {
    let mut base = miyu_base::config::AppConfig::default();
    let provider_id = base.providers[0].id.clone();
    let pool = |model: &str| {
        vec![miyu_base::config::ActiveProviderModelConfig {
            provider_id: provider_id.clone(),
            model: model.to_string(),
        }]
    };
    base.active_provider_models = Some(pool("global"));
    base.platforms.qq.text_models = miyu_base::config::ModelPoolRef::models(pool("platform"));
    base.platforms.qq.non_whitelist_text_models =
        miyu_base::config::ModelPoolRef::models(pool("non-whitelist"));
    ...
    for (target, user_id, expected) in [
        ...
        let admission = admission_for_access(&config.platforms.qq, None, target, 100, user_id);
...
fn dynamic_access_grants_feed_the_same_admission_matrix_for_every_bot() {
    ...
    let actor = miyu_core::state::PlatformAccessActor {
