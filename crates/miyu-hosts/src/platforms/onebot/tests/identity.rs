...
fn trusted_qq_mapping_binds_identity_without_trusting_the_nickname() {
    ...
    let settings = RealContextPluginSettings {
        identity_mappings: vec![miyu_base::config::RealContextIdentityMapping {
    ...
    config.plugins.insert(
        ...
        miyu_base::config::PlatformPluginInstanceConfig {
            enabled: Some(false),
...
async fn qq_conversation_persona_drives_context_and_session_binding() {
    ...
    config.platforms.qq.conversations[0].persona = miyu_base::config::PlatformPersonaOverride::Miyu;
