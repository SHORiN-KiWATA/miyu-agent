...
pub(crate) use miyu_base::platform_types::{
...
use miyu_base::i18n::{text_for, Locale};
...
impl PlatformRuntime {
    ...
    pub fn plugins(&self) -> Result<Arc<plugins::PlatformPluginRegistry>> {
        self.plugins
            .get_or_init(|| {
...
pub(crate) fn apply_platform_turn_scope(
    ...
    restricted_base: Option<&miyu_engine::tools::ToolRegistry>,
) {
    if !context.host_tools_allowed() {
        *registry = match restricted_base {
            Some(base) => base.clone(),
            None => miyu_engine::tools::restricted_platform_registry(config, paths),
        };
    } else {
        miyu_engine::tools::rescope_platform_memory_tools(registry, config, paths, context, false);
    }
