...
use miyu_base::config::RealContextPluginSettings;
    ...
    for attempt in 0..=settings.judge_max_retries {
        ...
        if let Some(usage) = result.usage.as_ref() {
            let meta = miyu_core::state::UsageMeta {
                ...
                // 统计里必须能单独看见(08-26)。
                kind: Some(miyu_core::state::USAGE_KIND_JUDGE),
            };
            if let Err(error) = context.state_store.add_auxiliary_usage(usage, meta) {
                tracing::warn!(error = %error, "{}", miyu_base::i18n::text("recording real-context judge usage failed", "记录真实上下文判断用量失败"));
            }
        }
...
fn build_prompt(
    ...
    let persona = judge_persona_prompt(settings, || {
        context
            .config
            .system_prompt_for(&context.paths, miyu_base::config::PromptAudience::Internal)
            .unwrap_or_default()
    });
