...
use miyu_engine::tools::ToolRegistry;
...
fn persona_overlay(
    ...
    let manifest = miyu_base::config::PersonaManifest::load(
        &context.config,
        &context.paths,
        ...
        Box::pin(async move {
            ...
            for (message_id, reaction_id) in reactions {
                ...
                if let Err(error) = context
                    ...
                    .await
                {
                    tracing::debug!(error = %error, %message_id, %reaction_id, "{}", miyu_base::i18n::text("aborted QQ reaction could not be removed", "无法移除已中止的 QQ 表情回应"));
                }
            }
            Ok(())
        })
    ...
    fn after_send<'a>(
        ...
        Box::pin(async move {
                ...
                && context.conversation.kind == ConversationKind::Group
            {
                ...
                {
                    tracing::warn!(target: "miyu::qq", error = %error, "{}", miyu_base::i18n::text("emotion update after reply failed", "回复后更新情绪状态失败"));
                }
