...
pub(in crate::agent) fn inline_media_from_tool_result(
    tool_name: &str,
    output: &str,
) -> Vec<miyu_core::state::TurnInlineMedia> {
    if let Some(image) = clipboard_binary_image_from_tool_result(tool_name, output) {
        return vec![miyu_core::state::TurnInlineMedia {
            call_id: String::new(),
            seq: 0,
            ...
            INLINE_MEDIA_KIND_VIDEO => {
                if let Some(url) = inline_media_url(item) {
                    parts.push(ChatContentPart::VideoUrl {
                        video_url: miyu_core::llm::VideoUrlContent { url },
                    });
                }
            }
            INLINE_MEDIA_KIND_PDF => {
                if let Some(url) = inline_media_url(item) {
                    parts.push(ChatContentPart::File {
                        file: miyu_core::llm::FileContent {
...
fn inline_media_parts(items: &[miyu_core::state::TurnInlineMedia]) -> Vec<ChatContentPart> {
    use miyu_core::state::{
        INLINE_MEDIA_KIND_IMAGE, INLINE_MEDIA_KIND_PDF, INLINE_MEDIA_KIND_TEXT,
        INLINE_MEDIA_KIND_VIDEO,
    };
    ...
    if tool_form {
        ...
        if !parts.is_empty() {
            let text = match tool_message.content.take() {
                Some(miyu_core::llm::ChatContent::Text(text)) => text,
                Some(miyu_core::llm::ChatContent::Parts(existing)) => existing
                    .into_iter()
            ...
            tool_message.content = Some(miyu_core::llm::ChatContent::Parts(content));
        }
        messages.push(tool_message);
        return;
    }
...
fn pdf_item_file_name(item: &miyu_core::state::TurnInlineMedia) -> String {
...
fn inline_media_url(item: &miyu_core::state::TurnInlineMedia) -> Option<String> {
    use base64::Engine;
    if let Some(data) = item.data.as_deref() {
...
pub(in crate::agent) fn rewrite_image_placeholders_with_paths(
    ...
    while let Some(start) = miyu_base::clipboard::MEDIA_PLACEHOLDER_PREFIXES
        .iter()
        ...
        .min()
    {
        ...
        if let Some(index) = image_placeholder_index(placeholder) {
            if let Some(Some(path)) = paths.get(index - 1) {
                // 标签原样保留,别把视频改写成图片。
                let label = miyu_base::clipboard::media_placeholder_prefix(placeholder)
                    .unwrap_or("[Image ")
                    .trim_start_matches('[')
                    .trim_end();
...
pub(in crate::agent) fn image_placeholder_index(placeholder: &str) -> Option<usize> {
    let inner = placeholder
        .strip_prefix(miyu_base::clipboard::media_placeholder_prefix(placeholder)?)?
        .strip_suffix(']')?
        .trim_start();
...
pub fn active_text_pool_supports_pdf(config: &AppConfig) -> bool {
    let choices = config.active_provider_model_choices();
    !choices.is_empty()
        && choices.iter().all(|choice| {
