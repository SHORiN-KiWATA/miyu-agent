...
fn queued_prompts_survive_prompt_changes_but_not_a_new_store_session() {
    ...
    store
        .reset_if_prompt_changed_with_compatible("system prompt one", None)
        .unwrap();
    store
        .enqueue_prompt("q1", "queued content", "queued", &[])
        .unwrap();
    store
        .reset_if_prompt_changed_with_compatible("system prompt two", None)
        .unwrap();
...
fn prompt_fingerprint_changes_never_delete_history() {
    let (_temp, store) = test_store();
    store
        .reset_if_prompt_changed_with_compatible("persona plus owner identity", None)
        .unwrap();
    store
    ...
    store
        .reset_if_prompt_changed_with_compatible("different persona", None)
        .unwrap();
    assert_eq!(store.load_visible_turns().unwrap().len(), 1);
}
...
fn prompt_fingerprints_are_isolated_per_session() {
    ...
    first_store
        .reset_if_prompt_changed_with_compatible("prompt A", None)
        .unwrap();
    second_store
        .reset_if_prompt_changed_with_compatible("prompt B", None)
        .unwrap();
    first_store
