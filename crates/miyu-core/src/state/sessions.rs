...
impl StateStore {
    ...
    pub fn platform_access_grants_if_authorized(
        &self,
        platform: &str,
    ...
    pub fn mutate_platform_access_grant_if_authorized(
        &self,
    ...
    pub fn with_platform_access_authorization<T>(
        &self,
        authorization: &PlatformAccessAuthorization,
    ...
    pub fn session_model_override(
        &self,
        session_id: &str,
    ) -> Result<Option<Vec<miyu_base::config::ActiveProviderModelConfig>>> {
        let Some(encoded) = self.conv_db.session_model_override(session_id)? else {
            return Ok(None);
        };
        let models =
            serde_json::from_str::<Vec<miyu_base::config::ActiveProviderModelConfig>>(&encoded)
    ...
    pub fn set_session_model_override(
        &self,
        session_id: &str,
        models: Option<&[miyu_base::config::ActiveProviderModelConfig]>,
    ) -> Result<()> {
    ...
    pub fn plugin_get_json_with_revision<T: serde::de::DeserializeOwned>(
        &self,
        scope: &PlatformPluginScopeKey,
        key: &str,
