...
impl OpenAiCompatibleClient {
    ...
    pub fn from_tier(
        config: &AppConfig,
        paths: &MiyuPaths,
        tier: miyu_base::config::ModelTier,
    ) -> Result<TierClient> {
    ...
    pub fn context_window_with_source(
        &self,
        config: &AppConfig,
    ) -> Result<Option<(usize, miyu_base::config::ContextWindowSource)>> {
        use miyu_base::config::ContextWindowSource;
